...
public class SimpleGradientBoostedTreeTransformer  extends AbstractTransformer {

    private static final Logger log = LoggerFactory.getLogger(com.andrew.sparkwork.trans.SimpleGradientBoostedTreeTransformer.class);

    public SimpleGradientBoostedTreeTransformer(Properties p) {
        super(p);
    }
    ...
    public Map<String, Dataset<Row>> transform(Map<String, Dataset<Row>> inputs, SparkSession session, String stepName) throws Exception {
        ...
        String trainName = props.getProperty("simpleGradientBoostedTree.trainDataFrame");
        if ( trainName == null) {
        ...
        String testName = props.getProperty("simpleGradientBoostedTree.testDataFrame");
        if ( testName == null) {
            throw new RuntimeException("You have to define which dataset to use in the train");
        }
        ...
        if( !props.containsKey("simpleGradientBoostedTree.labelColumn") ) {
            throw new RuntimeException("We need pipeline defined for MLLIB algorithm");
        }

        String labelColumn = props.getProperty("simpleGradientBoostedTree.labelColumn");

        List<PipelineStage> list = new LinkedList<>();

        if(  props.containsKey("simpleGradientBoostedTree.stringIndex" ) ) {

            String[] columns = props.getProperty("simpleGradientBoostedTree.stringIndex").split(",");
        ...
        if( !props.containsKey("simpleGradientBoostedTree.featureColumns") ) {
            throw new RuntimeException("We need to define features send to GradientBoostedTreeClassifier");
        }

        vectorAssembler.setInputCols(props.getProperty("simpleGradientBoostedTree.featureColumns").split(","))
                .setOutputCol("features");
        list.add(vectorAssembler);

        int maxIter = Integer.valueOf(props.getProperty("simpleGradientBoostedTree.params.maxIter", "20"));

        String predictionColumn = "predicted_"+labelColumn;
        GBTClassifier gbtClassifier = new GBTClassifier().setLabelCol(labelColumn)
                .setFeaturesCol("features").setPredictionCol(predictionColumn).setMaxIter(maxIter);

        list.add(gbtClassifier);
